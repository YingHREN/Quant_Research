...
1. 若 `outputs.registry.groups` 合法，则按 `order` 动态渲染；
2. 若注册契约缺失或损坏，则回退到当前四组，兼容旧预测缓存；
3. `cardinality=many` 读取数组，`cardinality=single` 将对象包装为单卡片；
4. 未知新组使用注册的 `label_key`，无需修改固定 `GROUPS`；
5. 每张卡展示决策权限，继续展示类型、生命周期和时点语义。
