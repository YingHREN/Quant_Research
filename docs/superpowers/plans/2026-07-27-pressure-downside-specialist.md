...
- [ ] 写失败测试，覆盖次日开盘、未来最低价、终点反弹、缺失路径和标签结束日。
- [ ] 运行 `../../venv/bin/python -m unittest tests.test_downside_specialist -v`，确认因接口缺失失败。
- [ ] 实现最小标签构建器。
- [ ] 重跑测试，确认标签测试通过。
- [ ] 提交 `research: add executable downside path labels`。
...
- [ ] 写失败测试，覆盖阶段门控、精确标签禁入、训练折预处理、分数范围和追加未来不改变旧预测。
- [ ] 运行目标测试并确认缺少训练接口。
- [ ] 实现类别平衡 Logistic 和固定 0.5 阈值。
- [ ] 重跑测试并确认通过。
- [ ] 提交 `research: add pressure-regime downside specialist`。
...
- [ ] 写失败测试，覆盖精确率、召回率、特异度、BA、ROC/PR AUC、Brier、非重叠样本和稀疏折排除。
- [ ] 运行目标测试并确认接口缺失。
- [ ] 实现指标、同池连接和冻结晋级门槛。
- [ ] 运行相关测试并确认通过。
- [ ] 提交 `research: evaluate downside specialist fairly`。
...
- [ ] 写失败测试，覆盖报告清单、中文结论、失败原因和输出完整性。
- [ ] 实现运行器与报告渲染。
- [ ] 运行 240 股票、2018-01-01 起点、五折真实实验。
- [ ] 检查所有组、阶段、周期和样本模式，冻结结论。
