...
- [ ] 写失败测试，使用手工官方形态样例验证代码大写、半开区间字段、退市标志和排序。
- [ ] 写失败测试，验证嵌套 `HistoricalTickerComponents`、空响应、非法日期、结束日不晚于开始日和冲突重复。
- [ ] 运行 `python -m unittest tests.test_point_in_time_universe -v`，确认接口缺失失败。
- [ ] 实现不可变数据类和严格标准化；完全相同的重复行去重，冲突重复抛出 `ValueError`。
- [ ] 重跑目标测试，确认通过。
- [ ] 提交 `data: normalize point-in-time universe feeds`。
...
- [ ] 写失败测试：已有旧版表可增加审计列且保留数据。
- [ ] 写失败测试：整体替换幂等、空记录拒绝、失败事务保留旧成员。
- [ ] 写失败测试：退市目录记录进入 `security_master`，代码变更幂等但不改写价格。
- [ ] 运行目标测试并确认缺少存储接口。
- [ ] 实现兼容列迁移、新代码变更表、最小证券目录 upsert 和事务写入。
- [ ] 重跑研究存储与标准化测试。
- [ ] 提交 `data: store historical universe intervals`。
...
- [ ] 写失败测试：加入日包含、移出日排除、退市股票在更早日期仍存在。
- [ ] 写失败测试：必须传 `asof`、未知股票池为空、批量日期与单日结果一致。
- [ ] 运行目标测试并确认读取接口缺失。
- [ ] 实现单次 SQL 区间读取和批量内存展开。
- [ ] 重跑读取、存储和标准化测试。
- [ ] 提交 `research: read point-in-time universe members`。
...
- [ ] 写失败测试：离线输入模式原子写三份 JSON，manifest 不含 token。
- [ ] 写失败测试：导入临时 SQLite 后按 2018/2020/当前日期生成覆盖审计。
- [ ] 实现带重试的 EODHD 采集和 `--input-components/--input-symbol-changes` 离线模式。
- [ ] 实现导入器与 JSON/Markdown 覆盖报告。
- [ ] 重跑 CLI 与全部相关测试。
- [ ] 尝试真实 EODHD 快照；若权限/网络不可用，保留明确阻塞状态而不伪造结果。
- [ ] 提交 `data: add point-in-time universe pipeline`。
...
- [ ] 更新中文 TODO，只关闭已获得真实证据的历史成员子项。
- [ ] 运行 `LOKY_MAX_CPU_COUNT=8 PYTHONWARNINGS=error venv/bin/python -m unittest discover -s tests -q`。
- [ ] 运行 `git diff --check` 并核对无 token、数据库 WAL/SHM 或临时文件进入提交。
- [ ] 合入 main，合并后再次运行完整测试。
