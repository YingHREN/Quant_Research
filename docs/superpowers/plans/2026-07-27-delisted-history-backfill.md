...
- [x] 在工作树运行全量测试、`git diff --check` 和密钥扫描。
- [x] 非快进合入 main，并在 main 再次运行全量测试。
- [x] 从 main 使用真实净化目录启动 13,039 只回填；每次进度更新核对处理数、
  下载数、空响应和错误数，网络中断时使用同一命令续跑。
- [x] 验证候选守恒、缓存哈希、质量统计、无密钥/临时文件和 Git 忽略状态。
- [x] 更新中文 TODO：记录真实可用历史、有效行数、体积和失败；只关闭正式
  原始回填，保留暂存库导入、历史行业与点时成员任务。
- [x] 提交真实报告，运行最终全量测试并记录完成。
